...
LOG_FILE_PATH = os.path.join(LOG_DIR, "responses_submitCase.log")
...
async def main_async():
    ...
    SUBMIT_PAYLOAD_TEMPLATE = load_json_from_path("3. dataSource/caseID.json")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        tasks = [
            asyncio.ensure_future(
                async_make_and_process_request(
                    session,
                    f"{ENV['BASE_URL']}/case-datasources/api/v1/cases/submit",
                    headers,
                    payload,
                )
            )
        ...
        with open(LOG_FILE_PATH, "w", encoding="utf-8") as log_file:
            for payload, (response_data, status_code, response_time) in zip(
                modified_payloads, responses
            ):
                case_id = payload.get(
                    "case_id", "Unknown"
                )  # Replace 'case_id' with the actual key in your payload
                print(f"Case ID: {case_id}, Status: {status_code}")
                log_entry = {
                    "Case ID": case_id,
                    "Status": status_code,
