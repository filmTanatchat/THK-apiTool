...
def main():
    ...
    PAYLOAD = load_json_from_path(
        os.path.join(BASE_PATH, "3. dataSource", "choiceCreateAndUpdate.json")
    )
    ...
    with requests.Session() as session:
        ...
        while not success and attempts < 10:
            result = make_request(
                session,
                f"{ENV['BASE_URL']}/universe/api/v1/update_choice",
                headers,
                PAYLOAD,
            )
            logs.append(result)
        ...
        with open(
            os.path.join(BASE_PATH, "2. log", "choiceUpdateLog.json"),
            "w",
            encoding="utf-8",
        ) as log_file:
