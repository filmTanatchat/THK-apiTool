...
def consistent_json_stringify(data):
    return json.dumps(data, sort_keys=True, separators=(",", ":"))


def main():
    ENV = load_json_from_path("1. environment/env.json")
    PAYLOAD = load_json_from_path("3. dataSource/caseID.json")
    ...
    with requests.Session() as session:
        ...
        with ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(
                    make_request,
                    session,
                    f"{ENV['BASE_URL']}/question-taskpool/api/v1/get-case-detail-all-task",
                    headers,
                    PAYLOAD,
                )
                for _ in range(10)
            ...
            for log in logs:
                response_str = consistent_json_stringify(log["Response"])
        ...
        with open(
            os.path.join(BASE_PATH, "2. log", "caseDetail.json"), "w", encoding="utf-8"
        ) as log_file:
